...
(function () {
  ...
  function onDOMContentReady() { // run when DOM is loaded
    ...
    * 功能：隐藏自己的审核被拒帖子
    */
    const hideRejectedPosts = () => {
      $('span.badge.badge-4').each((i, e) => {
        if ($(e).text().trim() === '审核被拒')
          $(e).closest('div.post').hide();
      })
    };
    if (settings.hideRejectedPosts) hideRejectedPosts();
